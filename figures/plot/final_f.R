...
if (F) {
  ...
  file_truths_generated <- "~/projects/as/data/sim/rsem/sim_1/truth.txt"
  truths_generated <- vroom(
    file_truths_generated, delim = "\t", col_types = paste(rep("c", 25), collapse = "")
  )
  truths_m <- full_join(truths_generated, truths, by = c("gene"  = "gene_id")) |>
    rename(gene_id = gene)
}
...
##### plot #####
dir_result <- "~/projects/as/analysis/filter/"

tools <- c(
  "rMATS", "CASH", "LeafCutter", "SplAdder", "MAJIQ", "SUPPA", "Whippet", "ASpli",
  "BANDITS", "NBSplice", "IsoformSwitchAnalyzeR", "psichomics", "DIEGO", "RATS",
  "dSpliceType", "PSI-Sigma", "JuncBASE",
  "DARTS_BHT", "DARTS_BHT_DNN", "JUM"
)
...
results$DIEGO <- results$DIEGO |>
  dplyr::select(!gene_id) |>
  left_join(tx2gene_convert, by = "gene_symbol")
results$ASpli <- results$ASpli |>
  dplyr::select(!c(gene_id, gene_symbol)) |>
  dplyr::rename(gene_id = locus) |>
  left_join(tx2gene_convert, by = "gene_id")


results$rMATS$idx <- results$rMATS$idx_truths
results$CASH$idx <- results$CASH$idx_truths_2
results$LeafCutter$idx <- results$LeafCutter$idx_truths
results$SplAdder$idx <- results$SplAdder$idx_truths
results$MAJIQ$idx <- results$MAJIQ$idx_truths
results$SUPPA$idx <- results$SUPPA$idx_truths
results$Whippet$idx <- results$Whippet$idx_truths_2
results$psichomics$idx <- results$psichomics$idx_truths
results$DIEGO$idx <- results$DIEGO$idx_truths
results$dSpliceType$idx <- results$dSpliceType$idx_truths_2
results$`PSI-Sigma`$idx <- results$`PSI-Sigma`$idx_truths_2
...
isoformswitchanalyzer_tmp <- results[["IsoformSwitchAnalyzeR"]][, c("gene_id", "gene_symbol", "myID", "padj", "SE", "MXE", "SME", "RI", "A5SS", "A3SS", "AFE", "ALE")]
isoformswitchanalyzer_tmp <- isoformswitchanalyzer_tmp |>
  mutate(
    ...
    SME = as.numeric(SME),
    RI = as.numeric(RI),
    A5SS = as.numeric(A5SS),
    A3SS = as.numeric(A3SS),
    AFE = as.numeric(AFE),
    ALE = as.numeric(ALE)
  )

number_detect <- lapply(
  tools[tools != "IsoformSwitchAnalyzeR"],
  function(x) {
    results[[x]] |>
      group_by(as_type) |>
      summarise(
...
number_detect$IsoformSwitchAnalyzeR <- tribble(
  ~as_type, ~g, ~e,
  "SE", 8804, 32984,
  "MXE", 171, 372,
  "SME", 4270, 11412,
...
number_detect_l <- number_detect |>
  ...
  ungroup() |>
  mutate(tool = fct_reorder(tool, e)) |>
  pivot_longer(cols = 2:3, names_to = "g_or_e", values_to = "nbr")


number_detect_l |>
  ggplot(aes(tool, nbr, fill = as_type)) +
  geom_col(
  ...
  scale_y_break(c(20800, 26800), scales = 0.2) +
  scale_y_break(c(56000, 109000), scales = 0.1) +
  theme_light() +
  theme(
...
percent_detection <- lapply(
  names(results[names(results) != "IsoformSwitchAnalyzeR"]),
  function(x) {
    results[[x]] |>
      summarise(
...
percent_detection$IsoformSwitchAnalyzeR <- tribble(
  ~g, ~e,
  0.0846584, 0.03039678
) |>
  mutate(tool = "IsoformSwitchAnalyzeR")
...
percent_detection_l |>
  ...
  scale_y_break(c(0.170, 0.6), scales = 0.2) +
...
percent_detection_as$IsoformSwitchAnalyzeR <- tribble(
  ~as_type, ~e, ~g,
  "SE", 0.03401649, 0.06667424,
  "MXE", 0.04301075, 0.07602339,
  "SME", 0.0242727, 0.0440281,
  "RI", 0.02465391, 0.04641076,
  "A5SS", 0.03833414, 0.08164243,
  "A3SS", 0.0398773, 0.0864512,
  "AFE", 0.02624021, 0.07604093,
  "ALE", 0.02378011, 0.07178571
) |>
  mutate(tool = "IsoformSwitchAnalyzeR")
...
percent_detection_as_l |>
  ...
  scale_alpha_manual(values = c(1, 0.3)) +
  scale_fill_manual(values = colors_as) +
  scale_y_break(c(0.177, 0.32), scales = 0.2) +
  # scale_y_break(c(0.4267, 0.8159), scales = 0.1) +
  theme_light() +
  scale_y_continuous(labels = percent) +
  theme(
    legend.key.size = unit(8, "pt"),
...
percent_detection_tf[[10]] <- tribble(
  ~t, ~f, ~tool, ~g_or_e,
  0, 1, "NBsplice", "g"
)
...
percent_detection_tf_e <- lapply(
  tools[!tools %in% c("ASpli", "BANDITS", "NBSplice", "IsoformSwitchAnalyzeR", "RATS")],
  function(x) {
...
##### tables #####
nbr_f <- read.xlsx("~/projects/as/analysis/filter/tables/nbr.xlsx") |>
  dplyr::rename(g_f = g, e_f = e)
nbr_o <- read.xlsx("~/projects/as/analysis/sim_1/tables/nbr.xlsx") |>
  dplyr::rename(g_o = g, e_o = e)
nbr <- full_join(nbr_f, nbr_o, by = c("tool", "as_type")) |>
  rowwise() |>
  mutate(
    g_s = g_o - g_f, e_s = e_o - e_f
  ) |>
  ungroup()

nbr_l <- nbr |>
  dplyr::select(!c(g_f, e_f, g_o, e_o)) |>
  bind_rows() |>
  arrange(desc(e_s)) |>
  group_by(tool) |>
  mutate(
    as_type = fct_reorder(as_type, e_s)
  ) |>
  ungroup() |>
  mutate(tool = fct_reorder(tool, e_s)) |>
  pivot_longer(cols = 3:4, names_to = "g_or_e", values_to = "nbr")


nbr_l |>
  ggplot(aes(tool, nbr, fill = as_type)) +
  geom_col(
    data = . %>% filter(g_or_e == "g_s"),
    position = position_dodge2(width = 2.2, padding = 0.2, preserve = "single"),
    alpha = 1, color = "black", size = 0.15, width = 1
  ) +
  geom_col(
    data = . %>% filter(g_or_e == "e_s"),
    position = position_dodge2(width = 2.2, padding = 0.2, preserve = "single"),
    alpha = 0.3, color = "black", size = 0.15, width = 1
  ) +
  scale_alpha_manual(values = c(1, 0.3)) +
  scale_fill_manual(values = colors_as) +
  scale_y_break(c(27000, 66000), scales = 0.2) +
  # scale_y_break(c(56000, 109000), scales = 0.1) +
  theme_light() +
  theme(
    legend.key.size = unit(8, "pt"),
  ...
  labs(fill = "Types of alternative splicing") +
  xlab("Tools") +
  ylab("Reduced number of genes / events") -> p
pdf(glue("{dir_result}/figs/nbr_s.pdf"), width = 8, height = 5, onefile = F)
print(p)
dev.off()
...
plot_fdr_tpr_paper(
  ...
  legend_nrow = 3, fig_height = 6, fig_width = 10
)
plot_fdr_tpr_paper(
  results, truths_m, tools, colors_tool, thresholds = c(0.01, 0.05, 0.1),
  ...
  legend_nrow = 2, fig_height = 6, fig_width = 15, long = F
)
...
plot_fdr_tpr_paper(
  ...
  legend_nrow = 3, fig_height = 8, fig_width = 12, long = F
)
plot_fdr_tpr_paper(
  results, truths_m, tools, colors_tool, thresholds = c(0.01, 0.05, 0.1),
  ...
  legend_nrow = 2, fig_height = 11, fig_width = 15, long = F
)
# plot_fdr_tpr_paper(
#   results, truths_m, tools, colors_tool, thresholds = c(0.01, 0.05, 0.1),
...
plot_fdr_tpr_paper(
  ...
  pathsize = 1, pointsize = 3, stripsize = 10, axistitlesize = 20, axistextsize = 15,
  legend_nrow = 3, fig_height = 9, fig_width = 10, long = F
)
plot_fdr_tpr_paper(
  results, truths_m, tools, colors_tool, thresholds = c(0.01, 0.05, 0.1),
  ...
  legend_nrow = 2, fig_height = 9, fig_width = 15, long = F
)
# plot_fdr_tpr_paper(
#   results, truths_m, tools, colors_tool, thresholds = c(0.01, 0.05, 0.1),
...
plot_tpr_paper(
  ...
  legend_nrow = 3, fig_height = 9, fig_width = 12, long = F
)
plot_tpr_paper(
  results, truths_m_fil, tools, colors_tool, thresholds = c(0.01, 0.05, 0.1),
  ...
  legend_nrow = 2, fig_height = 13, fig_width = 15, long = F
)
...
results$MAJIQ <- readRDS("~/projects/as/analysis/sim_1/MAJIQ/majiq2.rds")
results$NBSplice <- readRDS("~/projects/as/analysis/sim_1/NBSplice/nbsplice.rds")
results$JuncBASE <- readRDS("~/projects/as/analysis/sim_1/JuncBASE/juncbase.rds")
results$SpliceSeq <- readRDS("~/projects/as/analysis/sim_1/SpliceSeq/spliceseq.rds")
results$`VAST-TOOLS` <- readRDS("~/projects/as/analysis/sim_1/VAST-TOOLS/vasttools.rds")
...
##### save truths #####
if (F) saveRDS(truths_m, "~/projects/AS/data/sim/rsem/truths_m.rds")


##### extend #####
ds_gene_all <- sapply(tools[!tools %in% c("dSpliceType", "NBSplice", "JuncBASE")], function(x) {
  unique(results[[x]]$gene_id[as.numeric(results[[x]]$padj) <= 0.05]) # |> head(500)
}) |>
  flatten_chr() |>
  unique()
dat_ds_gene <- data.frame(matrix(0, nrow = length(ds_gene_all), ncol = length(tools[!tools %in% c("dSpliceType", "NBSplice", "JuncBASE")])))
colnames(dat_ds_gene) <- tools[!tools %in% c("dSpliceType", "NBSplice", "JuncBASE")]
dat_ds_gene$ds_gene <- ds_gene_all
dat_ds_gene <- dplyr::select(dat_ds_gene, ds_gene, everything())
for (t in tools[!tools %in% c("dSpliceType", "NBSplice", "JuncBASE")]) {
...
q_inte <- lapply(tools[!tools %in% c("dSpliceType", "NBSplice", "JuncBASE")], function(x) {
  tmp <- results[[x]] |> filter(as.numeric(padj) <= 0.05)
  qValue(tmp, "gene_id", "padj")
}) |>
  setNames(tools[!tools %in% c("dSpliceType", "NBSplice", "JuncBASE")])
...
for (at in c("SE", "RI", "A3SS", "A5SS", "MXE")) {
  pairs_tool <- pairs_tool_f <- matrix(
    nrow = length(names(results)[!names(results) %in% c("IsoformSwitchAnalyzeR", "BANDITS", "NBSplice", "DIEGO", "RATS", "LeafCutter")]), ncol = length(names(results)[!names(results) %in% c("IsoformSwitchAnalyzeR", "BANDITS", "NBSplice", "DIEGO", "RATS", "LeafCutter")])
  ) |>
    as.data.frame()
  colnames(pairs_tool) <- rownames(pairs_tool) <- names(results)[!names(results) %in% c("IsoformSwitchAnalyzeR", "BANDITS", "NBSplice", "DIEGO", "RATS", "LeafCutter")]
...
for (at in c("SE", "RI", "A3SS", "A5SS", "MXE")) {
  ...
  colnames(pairs_tool) <- rownames(pairs_tool) <-
    colnames(pairs_tool_f) <- rownames(pairs_tool_f) <- names(results)[!names(results) %in% c("IsoformSwitchAnalyzeR", "BANDITS", "NBSplice", "DIEGO", "RATS", "LeafCutter")]
  ...
  pairs_tool <- pairs_tool |>
    rownames_to_column("tool")
  write.xlsx(pairs_tool, glue("{dir_result}/tables/pairs.intersect.{at}.xlsx"))
}


for (at in c("SE", "RI", "A3SS", "A5SS", "MXE")) {
  ds_gene_all <- sapply(tools[!tools %in% c("dSpliceType", "NBSplice", "JuncBASE", "IsoformSwitchAnalyzeR", "BANDITS", "DIEGO", "RATS", "LeafCutter")], function(x) {
  ...
  dat_ds_gene <- data.frame(matrix(0, nrow = length(ds_gene_all), ncol = length(tools[!tools %in% c("dSpliceType", "NBSplice", "JuncBASE", "IsoformSwitchAnalyzeR", "BANDITS", "DIEGO", "RATS", "LeafCutter")])))
  colnames(dat_ds_gene) <- tools[!tools %in% c("dSpliceType", "NBSplice", "JuncBASE", "IsoformSwitchAnalyzeR", "BANDITS", "DIEGO", "RATS", "LeafCutter")]
  dat_ds_gene$ds_gene <- ds_gene_all
  dat_ds_gene <- dplyr::select(dat_ds_gene, ds_gene, everything())
  
  for (t in tools[!tools %in% c("dSpliceType", "NBSplice", "JuncBASE", "IsoformSwitchAnalyzeR", "BANDITS", "DIEGO", "RATS", "LeafCutter")]) {
...
for (at in c("SE", "RI", "A3SS", "A5SS", "MXE")) {
  q_inte <- lapply(tools[!tools %in% c("dSpliceType", "NBSplice", "JuncBASE", "IsoformSwitchAnalyzeR", "BANDITS", "DIEGO", "RATS", "LeafCutter")], function(x) {
    tmp <- results[[x]] |> filter(as.numeric(padj) <= 0.05, as_type == at)
    if (nrow(tmp) == 0) return(NULL)
    qValue(tmp, "gene_id", "padj")
  }) |>
    setNames(tools[!tools %in% c("dSpliceType", "NBSplice", "JuncBASE", "IsoformSwitchAnalyzeR", "BANDITS", "DIEGO", "RATS", "LeafCutter")])
