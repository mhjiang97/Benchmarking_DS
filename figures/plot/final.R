...
# results$MAJIQ$idx[results$MAJIQ$as_type == "OTHER"] <- NA
...
results$`VAST-TOOLS`$idx <- results$`VAST-TOOLS`$idx_truths
...
number_detect_l |>
  ...
  scale_y_break(c(19900, 23000), scales = 0.2) +
...
percent_detection_as$IsoformSwitchAnalyzeR <- tribble(
  ~as_type, ~e, ~g,
  "SE", 0.03037868, 0.06372707,
  "MXE", 0.04878049, 0.08441558,
  "SME", 0.02190637, 0.04325147,
  "RI", 0.02262917, 0.04782255,
  "A5SS", 0.03509823, 0.07986739,
  "A3SS", 0.0354805, 0.08229352,
  "AFE", 0.02361807, 0.07350097,
  "ALE", 0.02142245, 0.06929683
) |>
  mutate(tool = "IsoformSwitchAnalyzeR")
...
percent_detection_as_l |>
  ...
  scale_alpha_manual(values = c(1, 0.3)) +
  scale_fill_manual(values = colors_as) +
  scale_y_break(c(0.177, 0.25), scales = 0.2) + # 0.125
...
percent_detection_tf_e <- lapply(
  tools[!tools %in% c("ASpli", "BANDITS", "NBSplice", "IsoformSwitchAnalyzeR", "RATS", "SpliceSeq")],
  function(x) {
    tmp <- results[[x]]
...
) |> setNames(names(results[names(results) != "IsoformSwitchAnalyzeR"])) -> diff_nbr
# length(unique(isoformswitchanalyzer_tmp$gene_id[isoformswitchanalyzer_tmp$padj < 0.05]))
# colSums(isoformswitchanalyzer_tmp[isoformswitchanalyzer_tmp$padj < 0.05, 5:12]) |> sum()
diff_nbr$IsoformSwitchAnalyzeR <- tribble(
  ~g, ~e, ~tool,
  976, 6311, "IsoformSwitchAnalyzeR"
)
tbl_diff_nbr <- diff_nbr |> bind_rows()
write.xlsx(tbl_diff_nbr, glue("{dir_result}/tables/nbr_diff.xlsx"))

tf <- bind_rows(percent_detection_tf)
tf_e <- bind_rows(percent_detection_tf_e)
t <- tf_e$tool
tf$t[match(t, tf$tool)] - tf_e$t[match(t, tf_e$tool)] |> setNames(t) |> sort()
...
plot_fdr_tpr_paper(
  ...
  legend_nrow = 3, fig_height = 6, fig_width = 10, legendtextsize = 8
)
plot_fdr_tpr_paper(
  results, truths_m, tools, colors_tool, thresholds = c(0.01, 0.05, 0.1),
...
plot_fdr_tpr_paper(
  ...
  legend_nrow = 2, fig_height = 7, fig_width = 15, long = F
)
...
for (t1 in rownames(pairs_tool)) {
  for (t2 in colnames(pairs_tool)) {
    ...
    pairs_tool_f[t1, t2] <- round(tmp, 5)
  }
}
...
pairs_tool_f <- pairs_tool_f |>
  rownames_to_column("tool")
write.xlsx(pairs_tool_f, glue("{dir_result}/tables/pairs.intersect_f.xlsx"))
...
##### extend #####
ds_gene_all <- sapply(tools[tools != "dSpliceType"], function(x) {
  unique(results[[x]]$gene_id[as.numeric(results[[x]]$padj) <= 0.05]) # |> head(500)
}) |>
  flatten_chr() |>
  unique()
dat_ds_gene <- data.frame(matrix(0, nrow = length(ds_gene_all), ncol = length(tools[tools != "dSpliceType"])))
colnames(dat_ds_gene) <- tools[tools != "dSpliceType"]
dat_ds_gene$ds_gene <- ds_gene_all
dat_ds_gene <- dplyr::select(dat_ds_gene, ds_gene, everything())
for (t in tools[tools != "dSpliceType"]) {
...
q_inte <- lapply(tools[!tools %in% c("dSpliceType")], function(x) {
  tmp <- results[[x]] |> filter(as.numeric(padj) <= 0.05)
  qValue(tmp, "gene_id", "padj")
}) |>
  setNames(tools[!tools %in% c("dSpliceType")])
...
for (at in c("SE", "RI", "A3SS", "A5SS", "MXE")) {
  ...
  pairs_tool_f <- pairs_tool_f |>
    rownames_to_column("tool")
  write.xlsx(pairs_tool_f, glue("{dir_result}/tables/pairs.intersect.{at}_f.xlsx"))
}
