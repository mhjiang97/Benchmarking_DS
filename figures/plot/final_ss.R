...
##### plot #####
dir_result <- "~/projects/as/analysis/novel_splice_site/"

tools <- c(
  "rMATS", "CASH", "LeafCutter", "SplAdder", "MAJIQ", "Whippet", "psichomics", "JuncBASE",
...
results$rMATS$idx <- results$rMATS$idx_truths_3
results$CASH$idx <- results$CASH$idx_truths_6
results$LeafCutter$idx <- results$LeafCutter$idx_truths_3
results$SplAdder$idx <- results$SplAdder$idx_truths_3
results$MAJIQ$idx <- results$MAJIQ$idx_truths_3
results$Whippet$idx <- results$Whippet$idx_truths_6
results$psichomics$idx <- results$psichomics$idx_truths_3
results$JuncBASE$idx <- results$JuncBASE$idx_truths_3
results$DARTS_BHT$idx <- results$DARTS_BHT$idx_truths_3
results$DARTS_BHT_DNN$idx <- results$DARTS_BHT_DNN$idx_truths_3
results$JUM$idx <- results$JUM$idx_truths_5
...
number_detect_l |>
  ...
  scale_y_break(c(30000, 100000), scales = 0.2) +
  theme_light() +
  theme(
...
percent_detection_l |>
  ...
  scale_y_break(c(0.099, 0.31), scales = 0.2) +
...
percent_detection_as_l |>
  ...
  scale_alpha_manual(values = c(1, 0.3)) +
  scale_fill_manual(values = colors_as) +
  scale_y_break(c(0.188, 0.25), scales = 0.1) +
  scale_y_break(c(0.5, 0.85), scales = 0.1) +
  theme_light() +
  scale_y_continuous(labels = percent) +
  theme(
    legend.key.size = unit(12, "pt"),
...
percent_detection_tf_l |>
  ggplot(aes(reorder(tool, per), per, fill = t_or_f)) +
