...
if (F) {
  ...
  file_truths_generated <- "~/projects/as/data/sim/rsem/revision/sim_1/truth.txt"
...
truths <- readRDS("~/projects/AS/data/sim/rsem/truths.rds")
truths_m <- readRDS("~/projects/AS/data/sim/rsem/revision/truths_m.rds")

##### plot #####
dir_result <- "~/projects/as/analysis/revision/"
...
number_detect_l |>
  ...
  scale_y_break(c(10000, 25000), scales = 0.2) +
...
##### all #####
if (F) {
  truths_m$all <- "(n = 8723, n.ds = 1000, n.dse = 1723)"
}
...
if (F) {
  ...
  tmp <- read.delim(
    "~/projects/AS/data/sim/rsem/revision/sim_1/simulation_details.txt",
    header = T, as.is = T
  )
...
##### save truths #####
if (F) saveRDS(truths_m, "~/projects/AS/data/sim/rsem/revision/truths_m.rds")
...
q_inte <- lapply(tools[!tools %in% c("SingleSplice")], function(x) {
  tmp <- results[[x]] |> filter(as.numeric(padj) <= 0.05)
  qValue(tmp, "gene_id", "padj")
}) |>
  setNames(tools[!tools %in% c("SingleSplice")])
