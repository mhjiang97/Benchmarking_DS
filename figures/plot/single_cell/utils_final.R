pkgs <- c(
  ...
  "reshape2", "Hmisc", "purrr", "corrplot", "DEXSeq", "ggsci"
)
...
calc_FDR <- function(
  ...
  if (g_or_e == "g") {
    out <- lapply(tools, function(t) {
      if (t == "SingleSplice") thresholds <- 0.05
      
      tmp <- results[[t]] |>
        filter(gene_id %in% truths_m$gene_id)
      
      if (filter_as && t != "MicorExonator") {
        keep_as <- intersect(c("SE", "MXE", "A5SS/A3SS", "RI"), unique(tmp$as_type))
        if (length(keep_as) > 0) {
...
calc_TPR_as <- function(
  ...
  if (g_or_e == "g") {
    out <- lapply(tools, function(t) {
      ...
      if (!"as_type" %in% colnames(tmp)) return(NULL)
      keep_as <- intersect(c("SE", "MXE", "A5SS/A3SS", "A5SS", "RI"), unique(tmp$as_type))
      if (length(keep_as) > 0) {
        tmp <- tmp |> filter(as_type %in% keep_as)
        truths_m <- truths_m |> filter(as_type %in% keep_as)
      } else {
